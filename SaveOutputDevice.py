...
class SaveOutputDevice(OutputDevice):
    def __init__(self):
        ...
        self.setShortDescription(self._translations["save_tft_button]")
        self.setDescription(self._translations["save_tft_tooltip"])
    ...
    def requestWrite(self, nodes, file_name=None, limit_mimetypes=None, file_handler=None, **kwargs):
        ...
        dialog.setWindowTitle(self._translations["save_file_window"])
        ...
        if len(file_types) == 0:
            ...
            raise OutputDeviceError.WriteRequestFailedError(
                self._translations["no_file_warning"])
        ...
        if os.path.exists(file_name):
            result=QMessageBox.question(None, self._translations["file_exists_window"].format(file_name[file_name.rfind(
                "/")+1:]), self._translations["file_overwrite_label"].format(file_name[file_name.rfind("/")+1:]))
        ...
        try:
            ...
            message=Message(self._translations["file_saving_progress"].format(file_name),
                              0, False, -1, self._translations["file_saving_title"])
            message.show()
            ...
            raise OutputDeviceError.PermissionDeniedError(
                self._translations["permission_denied"].format(file_name)) from e
        except OSError as e:
            Logger.log(
                "e", "Operating system would not let us write to %s: %s", file_name, str(e))
            raise OutputDeviceError.WriteRequestFailedError(
                self._translations["permission_denied2"].format(file_name, str(e))) from e
    ...
    def _onWriteJobFinished(self, job):
        ...
        if job.getResult():
            ...
            message=Message(self._translations["file_saved_status"].format(
                job.getFileName()), title=self._translations["file_saved_title"])
            message.addAction(
                "open_folder", self._translations["open_folder_button"], "open-folder", self._translations["open_folder_tooltip"])
            message._folder=os.path.dirname(job.getFileName())
            message.actionTriggered.connect(self._onMessageActionTriggered)
            message.show()
        else:
            message=Message(self._translations["file_cant_save"].format(job.getFileName(
            ), str(job.getError())), lifetime=0, title=self._translations["warning"])
            message.show()
        ...
        # When you don't have the rights to do the final flush or the disk is full.
            ...
            message=Message(self._translations["file_something_wrong"].format(
                job.getFileName(), str(job.getError())), title=self._translations["error"])
            message.show()
            self.writeError.emit(self)
